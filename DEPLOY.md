...
**שמות אמיתיים.** הזריעה יוצרת `חניך 1`–`חניך 10`; תשעת השמות האמיתיים והשיבוץ מחדש
לזוגות מוכנים בקובץ `migrations/2026-09-assign-trainees.sql` — מריצים אותו **פעם אחת**
ב-SQL Editor של Supabase, אחרי הזריעה הראשונה (אפשר גם שוב — הוא אידמפוטנטי). הוא מעדכן
את תשעת השמות, מוחק את `חניך 10`, ומשבץ מחדש את משמרים 03–21 (38 מושבים: שבעה חניכים עם
ארבעה משמרים, שניים עם חמישה; אף אחד לא בשני משמרים רצופים). אין צורך להריץ שוב את
`supabase_schema.sql`. אותו שיבוץ כתוב גם ב-`Mishmer-section/2026-27/schedule.md`,
ב-`students.md` וב-`students_tasks.md` — הקובץ נוצר מ-`scripts/assign_trainees.py`, שמעדכן
את שלושתם יחד. חניך נכנס בהקלדת שמו **בדיוק** כפי שכתוב שם (התחברות פיתוח), או דרך גוגל
אחרי השיוך.
