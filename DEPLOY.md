...
## עדכון סכימה — גרסאות 7 ו-8

הריצו שוב את **כל** `supabase_schema.sql` ב-SQL Editor (בטוח להריץ שוב, בכל פעם). הרצה אחת
מכסה את שתי הגרסאות. **גרסה 8** מסירה פעם אחת את שתי המשימות הכלליות שנזרעו לכל משמר —
...
```sql
select value from app_meta where key = 'schema_version';   -- 8
