...
**לפני** שהשמות נכנסו (עם `חניך 1`–`חניך 10`) מתוקן פעם אחת ב-SQL Editor של Supabase עם
`migrations/2026-09-assign-trainees.sql` (אידמפוטנטי — הרצה חוזרת בטוחה): מעדכן את תשעת
השמות, מוחק את `חניך 10`, ומשבץ מחדש את משמרים 03–21 (38 מושבים: שבעה חניכים עם ארבעה
משמרים, שניים עם חמישה; אף אחד לא בשני משמרים רצופים). אין צורך להריץ שוב את
`supabase_schema.sql`. **לוודא שזה נקלט** — באותו SQL Editor:
