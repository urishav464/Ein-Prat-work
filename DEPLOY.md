...
**חיפוש ברשת צריך להיות פתוח בארגון של מפתח ה-API.** מסך «חיפוש מרצים» נותן למודל את כלי
החיפוש של אנתרופיק (‎$10 ל-1,000 חיפושים, עד 8 לסריקה) ואת כלי פתיחת הדפים. לפי התיעוד שניהם
**פתוחים כברירת מחדל**, אלא אם אדמין בארגון סגר אותם ב-
[platform.claude.com/settings/privacy](https://platform.claude.com/settings/privacy).
- חיפוש סגור → ה-API מחזיר 400 «web search is not enabled»; המסך אומר «חיפוש המרצים באינטרנט
  חסום כרגע בחשבון Anthropic של התוכנית — ספרו למדריך», מציע קישורי חיפוש ידני, ולמדריך
  (Uri) מוסיף את הקישור להגדרה.
- רק פתיחת דפים סגורה → הסריקה ממשיכה בחיפוש בלבד, ושורת העלות אומרת זאת.
- כל כשל אחר מוצג כ«הסריקה לא רצה», ותמיד עם «פרטים טכניים» — המילים של ה-API עצמו, שנשמרות
  גם ברשימת החיפושים בלוח הבקרה. הזיהוי של «סגור» נשען על הביטוי «not enabled» ולא על שם
  הכלי: הגרסה הראשונה זיהתה כל 400 שהזכיר את הכלי כ«סגור», והסתירה את הסיבה האמיתית.
...
## עדכון סכימה — גרסאות 7, 8 ו-9

הריצו שוב את **כל** `supabase_schema.sql` ב-SQL Editor (בטוח להריץ שוב, בכל פעם). הרצה אחת
מכסה את כל הגרסאות.

### גרסה 9

עמודה אחת — `mishmarim.start_time` — שעת הפתיחה של הערב. כל שעות המקטעים נגזרות ממנה, וקודם
היא הייתה קבועה בקוד על 20:00, כך שערב שמתחיל ב-20:30 לא היה ניתן לביטוי. ברירת המחדל היא
`'20:00'`, ולכן במסד קיים שום ערב לא זז. משנים אותה **רק מלוח הבקרה של המדריך**, בתפריט
«🕗 שעת ההתחלה של הערב» — שם בוחרים משמר ושעה, ואפשר להחיל על כל המשמרים שטרם התקיימו.
שינוי השעה מחשב מחדש את כל שעות המקטעים של אותו ערב.

**גרסה 8** מסירה פעם אחת את שתי המשימות הכלליות שנזרעו לכל משמר —
«סגירת מרצים» ו«סגירת חברותות/חבורות» — שהיו כפילות של המשימות שמבנה הערב יוצר לכל מקטע,
ושאף אחד לא סגר, ולכן שלב «מרצים ותוכן» לא נסגר לעולם. מה שסומן «בוצע» נשאר. «סידור חדרים»
מקבל את הניסוח של התבנית («סידור הבית מדרש»).
...
```sql
select value from app_meta where key = 'schema_version';   -- 9
select id, start_time from mishmarim order by id limit 3;  -- 20:00
select generated, count(*) from tasks group by 1;
select count(*) from tasks where task_description in ('סגירת מרצים','סגירת חברותות/חבורות') and status <> 'DONE';  -- 0
```

## שני דברים שחייבים לקרות לפני שהחניכים נכנסים

**שמות אמיתיים.** מסד **חדש** נזרע ישירות עם שמונת החניכים ו-38 השיבוצים — הזריעה קוראת
אותם מטבלת האינדקס ומשורות «אחראים» ב-`students_tasks.md`, ואין צורך בשום SQL.

**וכשהשיבוץ משתנה אחר כך** — חניך עוזב, הזוגות מצוירים מחדש — מיישרים את המסד לקובץ
**מתוך האפליקציה**: בלוח הבקרה של המדריך יש תפריט **«👥 חניכים ושיבוץ»**, תמיד, שמראה זה
לצד זה מה כתוב בקובץ ומה יש במסד ומחזיק את הכפתור «החל את השמות והשיבוץ מהקובץ». כשיש
פער, מופיע מעליו גם כרטיס שמפרט בדיוק מה שונה — אילו משמרים, מי נוסף ומי ירד. לוחצים,
קוראים את מה שעומד להשתנות, מאשרים בתיבת הסימון, וזהו. ההחלה מוסיפה שם חדש, **מוחקת חניך
שכבר לא מופיע בקובץ** (השיבוצים שלו נמחקים איתו; מה שכתב נשאר בלי שם כותב), ומשבצת מחדש
את משמרים 03–21; משמרי הצוות, המשימות, המרצים ומבנה הערבים לא נוגעים, והרצה חוזרת לא
משנה דבר.

> הכפתור הזה היה קודם גלוי רק כשהמסד עוד היה עם `חניך N` — כלומר נעלם אחרי השימוש הראשון,
> ושינוי מאוחר יותר בזוגות לא היה לו שום דרך להיכנס. עכשיו התפריט קיים תמיד.

אותו שינוי בדיוק אפשר להריץ גם ב-SQL Editor עם `migrations/2026-09-assign-trainees.sql`
(עדיף אם משהו באפליקציה לא נטען). אין צורך להריץ שוב את `supabase_schema.sql` בשביל
השיבוץ. **לוודא שזה נקלט** — ב-SQL Editor:
...
הצפוי: שמונה שמות (ו-Uri ב-100), ושתי שורות לכל משמר 03–21. אם לוח הבקרה של המדריך מציג
