...
8. **כל פנייה למרצה נרשמת ביומן המשותף.** כמה זוגות מחפשים מרצים במקביל;
   יומן שלא מתעדכן הוא איך ששני זוגות פונים לאותו אדם בלי לדעת. לפני שאתה
   מציע מרצה — בדוק אם כבר פנו אליו.
9. **מבנה הערב הוא משטח העבודה המרכזי — שקף אותו במדויק.** כל שינוי שהחניך
   מספר עליו — משך, מועמד, מרצה שנסגר, דף מקורות — בצע בכלי, ואמור בדיוק מה
   השתנה בלוז.
...
TOOLS: list[dict] = [
    {
        ...
        "description": (
            ...
            "השתמש רק אחרי שהחניך אישר במפורש את הניסוח. מחזיר `phase_opened` "
            "(השלב שנפתח ומשימותיו הפתוחות) ו-`index_matches` (התאמות מהמאגר, "
            "כולל מי שכבר פנו אליו), ובונה לוז ברירת מחדל אם הערב ריק. באותה "
            "תשובה: ברך בקצרה, הצג את המשימות שנפתחו, נקוב בהתאמות כנקודת פתיחה, "
            "והצע שני המשכים — לפרט משימות, או חיפוש רשת לשמות חדשים."
        ),
        "input_schema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": (
            ...
            "הפסקות הן משבצות לוז רגילות עם is_break. קרא כשהחניך מספר על "
            "מקטע חדש או על שינוי במשך. מחזיר את הלוז המחושב מחדש — דווח את "
            "השעות ממנו."
        ),
        "input_schema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": (
            "מוסיף מרצה אופציונלי לשיעור (מועמד — עוד לא נסגר). קרא כשהחניך "
            "מציע או שוקל מרצה לשיעור מסוים. השם נלמד אוטומטית גם במאגר "
            "המשותף. טלפון — רק אם החניך מסר אותו."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            ...
            "המשותף, ושאר המועמדים של השיעור מוסרים. קרא לזה כשהחניך מספר "
            "שמרצה סגר סופית — הרישום ביומן כלול, אין צורך ב-record_speaker_outreach. "
            "אם יש במאגר כמה אנשים בשם הזה, המרצה נסגר לשיעור אבל לא נרשם ביומן, "
            "והתוצאה אומרת זאת."
        ),
        "input_schema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": (
            "שומר קישור לדף המקורות של שיעור — קרא כשהחניך מספר שהמרצה שלח "
            "קובץ או קישור. url הוא הקישור שהחניך מסר, לא כתובת משוערת; שמירה "
            "מחליפה קישור קודם של אותו שיעור."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        "name": "update_task",
        "description": (
            "מעדכן סטטוס של משימה קיימת. task_id הוא המזהה שמופיע ליד המשימה "
            "בהקשר — משימה של משמר אחר תידחה. הכלי לא יוצר ולא מוחק; משימה "
            "חדשה — add_task."
        ),
        "input_schema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": (
            ...
            "שאף אחד לא הכיר. איטי (יש השהיה מכוונת) ומוגבל-קצב — הצע אותו "
            "לחניך והרץ כשהוא רוצה שמות חדשים. כשהחיפוש נחסם, מחזיר קישורים "
            "לחיפוש ידני במקום שמות."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        "name": "verify_speaker",
        "description": (
            "מריץ את צ'קליסט ⚠️ לאמת על שם: חי? פעיל? איפה? כבר פנינו אליו? "
            "איטי (שני חיפושי רשת עם השהיה מכוונת). מחזיר ממצאים וקישורים, לא "
            "פסק דין — הצג אותם לחניך כראיות."
        ),
        "input_schema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": (
            "רושם ביומן המשותף שפנינו למרצה ומה קרה. קרא לזה כשהחניך מספר "
            "שפנה למרצה או שקיבל תשובה — כך זוג אחר יודע שלא לפנות לאותו אדם. "
            "מרצה שנסגר סופית — close_speaker, שרושם ביומן בעצמו. אם יש כמה "
            "אנשים באותו שם, הכלי יחזיר את האפשרויות ותצטרך לשאול את החניך "
            "במי מדובר."
        ),
        "input_schema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": (
            "מה ידוע על מרצה מהעבר — רשומות המאגר בשם דומה (מה מלמד, סטטוס) "
            "ושורות משוב עם דירוג. כמה רשומות בשם דומה הן אנשים שונים עד "
            "שהחניך אומר אחרת. תוצאה ריקה פירושה שאין תיעוד, לא ביקורת שלילית."
        ),
        "input_schema": {
            "type": "object",
...
# The map's shape, enforced by structured outputs. This call has no tools and
# no citations, so the API can guarantee it; the scan below cannot use it —
# web search returns citations, which structured outputs reject.
MAP_SCHEMA = {
    "type": "object",
    "properties": {
        "reading": {"type": "string"},
        "angles": {"type": "array", "items": {
            ...
            "required": ["key", "label", "field", "who", "terms", "where", "why"],
            "additionalProperties": False,
        }},
    },
    "required": ["reading", "angles"],
    "additionalProperties": False,
}
...
7. עברית בלבד. טקסט בין החיפושים לא מוצג לאיש ולא נקרא — רק ה-JSON שבסוף.
   החזר JSON בלבד, בלי הקדמה ובלי סיכום. `bio` ו-`fit` — משפט אחד כל אחד.
...
`region_hint` — עיר או אזור רק אם הם כתובים בדף שקראת; המרחק מהמדרשה מחושב
אצלנו מתוך השיוך והאזור.
"""
...
def scout_map(topic: str, lesson_topic: str = "", angle: str = "") -> dict:
    ...
    try:
        ...
        if getattr(resp, "stop_reason", None) == "max_tokens":
            raise ValueError("the map was cut off at max_tokens")
