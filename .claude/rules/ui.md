...
run. After the reviewer's first full audit (cached season lists instead of per-id reads, TTL 900 s,
the dashboard's overdue list derived, one `assignments` and one `students` read, no feedback read
before the evening): **a cold workfile 10 → 5 queries (1.7 → 0.9 s); the home's cold load
including login 9 (4 of them the once-per-process bootstrap); ✓ anywhere = 2 queries.** The `app-reviewer` agent drives all of this; `deploy-check` runs the sweep as its click
budget.
