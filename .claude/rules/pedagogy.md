...
The digest in the generator prompt must never contain phone numbers or emails — it travels to external chat windows. **The app's search screen no longer mines names itself** — `chat_agent.scout_map` reads the topic as fields and `scout_speakers` lets the model search the web (see `chat-agent.md`). `speaker_search.py` keeps two paths for the CLI agents (the screen's «אמת» became a link to the institutional page): `search_candidates()` **discovers** (mines names from broad DuckDuckGo queries), `verify_speaker()` **verifies** one name against the `⚠️ לאמת` checklist. On that path, **the burst, not the volume, trips DuckDuckGo's limiter** (~25 queries in 3 minutes), hence: Supabase cache (60 days success, **1 hour failure** — or one blocked afternoon poisons the cache for the season), a 4s module-level throttle, a 60s→5m→15m cooldown ladder, backend rotation, and soft-fail to clickable manual-search links. Search the `notes` column, not just `expertise_topics` — "מה העביר אצלנו" lands in notes and is often the strongest topical signal.
...
**(היסטוריה — הצינור שקדם למפה; חי היום רק בסוכני ה-CLI.)** **החיפוש לא מוותר עד ארבעה שמות חזקים — אבל לא מרפד.** `search_candidates` רץ בסבבים: זווית → מוסדות (ac.il · הרטמן · ון ליר · בית אבי חי · הרצוג · שזר) → פעילות (ראיון · הרצאה 2025-26 · פודקאסט), ועוצר כשיש `MIN_STRONG_CANDIDATES` בוודאות גבוהה או כשתקציב `MAX_DISCOVERY_QUERIES` נגמר. אז מגיע **העמקה**: `verify_speaker(depth=2)` על ששת הראשונים, והעלאת ודאות רק על סמך **עמוד מוסדי + פעילות עדכנית** — ראיה, לא ניחוש. כשנמצאו פחות מארבעה, המסך אומר כמה נמצאו ואחרי כמה סבבים; **רשימה מרופדת גרועה מרשימה קצרה וכנה**. **החיפוש הראשון בשטח (פוסטמודרניזם, «מות הנראטיב הגדול…») החזיר 38 «שמות» שאף אחד מהם לא אדם.** שתי סיבות, שתיהן תוקנו ב-`speaker_search.py`: (1) כותרת השיעור המלאה נכנסה לכל שאילתה **במרכאות** — ביטוי מדויק שאף דף ברשת לא מכיל, והמנוע התדרדר לרעש. עכשיו רק הנושא במרכאות, וכותרת השיעור הופכת לשלוש מילות תוכן לא-מצוטטות (`lesson_keywords`) בסבב הראשון בלבד. (2) «שם חשוף» היה כל שתיים-שלוש מילים עבריות שלא ברשימה שחורה קצרה — «עבודה נוספת לא», «גולשים לתוכן ההרצאה». עכשיו שם חשוף הוא צמד מילים סמוכות בכותרת (`_bare_windows`, כל צמד נבדק — רגקס לא-חופף התיישר על «שיחה עם דן» ולא ראה את «דן כהן»), שצריך **אות שנייה**: אותו צמד בתוצאה נוספת, או מילת תפקיד צמודה («שיחה עם», «מרצה», «מאת»); מילות הנושא עצמן לעולם לא שם; רשימת מילות עצירה של ~150 מילים. צמד בודד מכותרת אחת נזרק. `low` פירושו עכשיו «חשוף אבל מאומת», ולכן כולם נשלחים לסינון.
`_trim_to_name` נוסף כי שני הביטויים הרגולריים תופסים עד שלוש מילים, ו«ד״ר רות לוי מרצה» נזרק כולו — מה שהוריד אדם אמיתי מ«גבוהה» ל«בינונית» או החוצה לגמרי.

**הפעולה על כרטיס מועמד היא «הוסף כמועמד», לא «שבץ»** — הוא נכנס ל-`lesson_speakers` (ולמאגר המשותף), והסגירה נשארת בקובץ העבודה דרך `close_lesson_speaker`. שיבוץ ישיר מהחיפוש הפך את השם הסביר הראשון להחלטה.

**מסך «חיפוש מרצים» סורק את הרשת בלבד** — המאגר המשותף אינו מחופש שם, כי המסך נועד למצוא מי שעוד לא מוכר. מה שכן נבדק הוא **התנגשות**: אם השם כבר במאגר או שכבר פנו אליו השנה, הכרטיס אומר זאת. הסינתזה מחזירה חמישה שמות עם שיוך מוסדי, אזור, תיאור-שורה וקישור למאמר/ראיון — **וכל שדה שאין לו ביסוס בתוצאות שנשלחו חוזר ריק**: `link` חייב להיות אחד הקישורים שנשלחו, ופרטי קשר נמחקים מהתשובה בקוד, לא מתבקשים בפרומפט. הזווית (יסודות · ערעור/טוויסט · זווית מפתיעה) היא **המלצה** שברירת המחדל שלה ריקה — בלי בחירה נסרקות כל הזוויות.
