...
def build_help(wb):
    ...
    steps = [
        ...
        ("•", "שינית שם של קבוצה ב«קבוצות»? עדכן אותו גם ב«שיבוץ» (חיפוש והחלפה). עמודת «בדיקה» שם תדליק אזהרה כתומה על כל שורה עם שם קבוצה שאינו קיים."),
...
def build_assign(wb):
    ...
    page(ws, landscape=True)
    widths(ws, {"A": 24, "B": 14, "C": 44, "D": 60, "E": 10, "F": 22, "G": 24})
    title_row(ws, 1, "שיבוץ תורנויות לשבת", span="A:E", size=18)
    header_row(ws, 2, ["קבוצה", "חלון זמן", "משימה", "פרטים וכמויות", "בוצע",
                       "מפתח (אל תיגעו)", "בדיקה"])
    ...
    for i in range(ASSIGN_ROWS):
        ...
        check = ws.cell(
            ...
            value='=IF($A{r}="","",IF(COUNTIF(\'{g}\'!$A$3:$A${gl},$A{r})=0,'
                  '"⚠ שם קבוצה לא מוכר",""))'.format(r=r, g=SH_GROUPS, gl=2 + MAX_GROUPS),
        )
        check.font = f(10, bold=True, color="8A4B00")
